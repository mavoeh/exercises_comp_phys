...
def m_breakup(q0, pf_ray, qf_ray, m):
  ...
  Jacobi momenta pf_ray and qf_ray of particles in the final state
  mass m 
  all in fm**-1
  '''
...
def breakup_cross(Elab, k1, k2, theta1, theta2, phi12, m = 938.92):
  ...
  Elab    -   Energy of the incoming particle in fm**-1
  k1      -   Abs. value of first particles's momentum after scattering
  k2      -   Abs. value of second particles's momentum after scattering
  ...
  returns: sigma - differntial breakup cross-section in the lab frame dsigma/(dO1dO2dS)
  ...
  #first: calculate p  = 1/2*(k_1- k_2), q = 2/3*k_3 - 1/3*(k_1+k_2) in fm**-1
  ...
  #from momentum conservation get k_3
  k_3 = k_lab- k_1 -k_2
  ...
  #amplitude 
  amp = np.absolute(m_breakup(q0, p, q, m/scattL0.hbarc))**2

  k1k2 = np.sin(theta1)*np.sin(theta2)*np.cos(phi12) + np.cos(theta2)*np.cos(theta1)

  sigma = (2*np.pi)**4 * amp * (m/scattL0.hbarc)**2 * k1**2 * k2**2 * 2.*(m/scattL0.hbarc)/3./q0 / \
...
S, kx, ky = scurve(theta1,theta2, phi, Elab, deg=True)
S = S[::1000]
kx = kx[::1000]
ky = ky[::1000]
...
for i in range(len(S)):
  sigma[i] = breakup_cross(Elab, kx[i], ky[i], theta1,theta2, phi)
